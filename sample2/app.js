...
app.post("/modifyPro", (req, res) => {
    ...
    var bno = req.body.bno; // const 가능
    const title = req.body.title;
    const content = req.body.content;

    var sql = `update board set titile=?, content=? where bno=?`
    var values = [title, content, bno]

    conn.query(sql, values, function(err, result) {
        if(err) throw err;
        res.send("<script>alert('수정완료'); location.href='/list';</script>")
    })
})
...
app.get("/delete", (req,res) => {
    var bno = req.query.bno;
    var sql = `delete from board where bno = ${bno}` // 백틱 안에 변수 사용
    // var sql = 'delete from board where bno =' + bno
    conn.query(sql, function(err,result) {
        if(err) throw err;

        res.send("<script>alert('삭제 완.');location.href='list';</script>")
    })
})
